...
<template>
  <section>
    <SectionLabel class="mb-3">{{ t("devices.title") }}</SectionLabel>
    <Panel class="space-y-4 p-6">
      ...
      <div v-if="devicesStore.loading" class="space-y-3">
        <Skeleton v-for="i in 2" :key="i" class="h-24 w-full rounded-lg" />
      </div>
      ...
      <div v-else class="space-y-4">
        <div
          ...
          :key="device.id"
          class="rounded-lg border border-separator bg-muted/30 p-5 space-y-4"
        >
          <!-- Header: info + actions -->
