...
- [x] **B11 — Tests agent** (`tests/agent/`). *(L'essentiel des sujets visés était déjà couvert par les tests écrits au fil de B1–B10 : logique delta (`test_sync_command.py`), cache de hash + invalidation mtime/size (`test_hash_cache.py`), installer sur FS isolé en `tmp_path` — l'équivalent « dry-run » : jamais le vrai `~/Library/Fonts` (`test_font_installer.py`), reconnexion `listen` après coupure (`test_listen_command.py`). B11 ferme les **derniers trous de couverture** des modules sans test dédié : `test_discovery.py` (scan dossiers — filtrage extension/casse, récursion, patterns d'exclusion, dédup, dossier absent, `expanduser` ; bascule `discover_fonts` Core Text↔dossiers ; repli `[]` sans pyobjc via `__import__` bloqué), `test_hashing.py` (chemin `scan_fonts` **sans** cache, `hash_file` vs `hashlib` y c. multi-chunk >64 Ko, font illisible ignorée non bloquante, callback `on_progress`), `test_cli.py` (dispatch `agent/__main__.py` : défaut→`sync`, routage `sync`/`listen`/`setup`/`teardown`/`status`, propagation du code de sortie, commande inconnue→`SystemExit`). +23 tests → **99 tests agent verts** ; `ruff format`/`check` OK.)*
